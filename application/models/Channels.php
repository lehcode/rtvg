...
class Xmltv_Model_Channels extends Xmltv_Model_Abstract
{
     ...
     *
     * @param  string $alias
     * @param  bool $withPlayer //Load player data?
     * @return stdClass
     */
    public function getByAlias($alias = null, $player = false, $news = false)
    {
        ...
        if ((bool) $news === true) {
            $select->joinLeft(array('NEWS' => 'rtvg_channels_news'), "`CH`.`id` = `NEWS`.`channel`", array(
                'rss_url' => 'url',
                'rss_enabled' => 'active',
            ));
        }
        ...
        if ((bool) $player === true) {
            $select->joinLeft(array('TORRENTTV' => 'rtvg_ref_streams_torrtv'), '`CH`.`id` = `TORRENTTV`.`channel`', array(
                    'torrenttv_id' => 'stream'
                ))
                ->joinLeft(array('TVFORSITE' => 'rtvg_ref_streams_tvforsite'), '`CH`.`id` = `TVFORSITE`.`channel`', array(
                    'tvforsite_id' => 'stream'
        ...
        $result = $this->db->fetchRow($select);
        $result['torrenttv_id'] = (isset($result['torrenttv_id']) && $result['torrenttv_id'] !== null) ? (int) $result['torrenttv_id'] : null;
        $result['tvforsite_id'] = (isset($result['tvforsite_id']) && $result['tvforsite_id'] !== null) ? $result['tvforsite_id'] : null;
        $result['id'] = (int) $result['id'];
        $result['free'] = (bool) $result['free'];
        $result['featured'] = (bool) $result['featured'];
        $result['category_id'] = (int) $result['category_id'];
        $result['rss_enabled'] = isset($result['rss_enabled']) ? (bool) $result['rss_enabled'] : null;
