...
class Zend_View_Helper_FeaturedChannelsList extends Zend_View_Helper_Abstract
{
	function featuredChannelsList(){
		...
		$html='<ul id="featured_channels">'; 
		for ( $i=0; $i<count($list); $i++ ) {
			if (($i%$cols==$cols-1) || $i==(count($list)-1)) {
				$html .= '<li class="lastcol"><a href="/телепрограмма/'.Xmltv_String::strtolower($list[$i]['alias']).'">Программа '.$list[$i]['title'].'</a></li>';
			} else {
				$html .= '<li><a href="/телепрограмма/'.Xmltv_String::strtolower($list[$i]['alias']).'">Программа '.$list[$i]['title'].'</a></li>';
			}
		}
